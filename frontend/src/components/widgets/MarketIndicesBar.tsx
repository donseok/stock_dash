...
export function MarketIndicesBar() {
  ...
  if (isLoading || !indices) {
    return (
      <div className="bg-slate-900 dark:bg-slate-950 text-white h-10 flex items-center px-4">
  ...
  return (
    <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-white h-10 flex items-center overflow-x-auto">
      <div className="flex gap-6 px-4 min-w-max">
        {indices.map((index) => (
          <div key={index.symbol} className="flex items-center gap-2 text-xs">
            ...
            <span
              className={`px-1.5 py-0.5 rounded text-[11px] font-medium ${index.change > 0
